...
const NetSet = () => {
  const [netset, setNetset] = useState({});

  const handleNetset= async () => {
    const data = { slug: "net-set" };
    try {
      const resp = await getTelecommunicationBySlug(data);
      console.log(resp);
      setNetset(resp && resp.data.data);
        ...
        <Row>
          <Col md={12} className="mt-2">
           ...
           privateNetwork.map((data) => { */}
            ...
            <div>
              <h4 className="mt-5 mb-5" style={{ color: "#1D3557" }}>{netset.name}</h4>

              <p>{netset.description}</p>
            </div>
            <div>
              {/* <li>{data.list1}</li>
              <li>{data.list2}</li>
